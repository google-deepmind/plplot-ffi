...
function tests.testLine()
  ...
  -- Render cairo surface as a tensor
  local result = cairo.rgb2tensor(cairoSurface, false):double():div(255)

  -- Open reference PNG
  local pathReference = paths.thisfile('../img/line.png')
  local reference = image.load(pathReference)

  tester:assertTensorEq(result, reference, 1e-15, 'Different from reference')
end
